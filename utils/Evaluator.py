...
if __name__ == "__main__":
    ...
    datasets = [("multi_session_chat_prev", 16300), 
    ...
    for (dataset, total_instances) in datasets:
        ...
        for ext in EXT:
            temp_files = list(filter(lambda f: f.endswith(EXT), os.listdir(dir_path)))
        ...
        for input_file in files:
            try:
                ...
                if "all" in args.compute:
                    res, eval_instances, logs = my_eval.compute(os.path.join(dir_path, input_file), metrics=all_metrics)
                elif "gpu" in args.compute:
                    res, eval_instances, logs = my_eval.compute(os.path.join(dir_path, input_file), metrics=all_metrics[:3])
