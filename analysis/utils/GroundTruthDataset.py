...
class GroundTruthTopical(BaseDataset):
    ...
    def __init__(self, utterance_filename, reading_set_filename=None, tokenizer=None, length=None):
        with open(utterance_filename, 'r') as f:
            temp_data = json.load(f)
        
        reading_set = [None] * len(temp_data)
        if reading_set_filename is not None:
            with open(reading_set_filename, 'r') as f:
                reading_set = json.load(f)
            
        super().__init__(tokenizer)
        self.data = self._construct(temp_data, length, reading_set)
    ...
    def _process_article(self, article):
        ...
        for k, v in article.items():
            if "headline" in k or "url" in k:
                continue
            content += f" {k}: {v}\n"
        
        return f"HEADLINE: {headline}\nCONTENT: {content}"
    
    def _process_fun_fact(self, fun_fact):
        content = ""
        
        for k, v in fun_fact.items():
            cur = f"{k}: "
            if 'entity' in v.keys():
                cur += f"ENTITY: {v['entity']}\n"
            
            if 'shortened_wiki_lead_section' in v.keys():
                cur += f"SHORTENED WIKI: {v['shortened_wiki_lead_section']}\n"

            if 'fun_facts' in v.keys():
                all_facts = "\n".join(v['fun_facts'])
                cur += f"FUN FACTs: {all_facts}\n"
            
            content += cur
        return content
    
    def _construct_reading_set(self, reading_set):
        if reading_set is None:
            return None
        article_material = self._process_article(reading_set["article"])
        agent1_fun_fact = self._process_fun_fact(reading_set["agent_1"])
        agent2_fun_fact = self._process_fun_fact(reading_set["agent_2"])
        return f"ARTICLE: {article_material}\nAGENT1 WIKI: {agent1_fun_fact}\nAGENT2 WIKI: {agent2_fun_fact}"
    
    # override
    def _construct(self, data, length=None, reading_set=None):
        ...
        data = data.values()
        reading_set = reading_set.values()

        for example, reading_set in zip(data, reading_set):
            
            reading_set_material = self._construct_reading_set(reading_set)            
            ...
            for i in range(1, len(utterances)):
                ...
                if reading_set_material is not None:
                    base_obj['reading_set'] = reading_set_material # for concatenating
                    base_obj['exact_reading_set'] = reading_set # for easier preprocessing
...
if __name__ == "__main__":
    base_path = "/home/bsantra/abhi/Budget-Constrained-Prompts-for-Dialog-Systems/data/topical_chat/datafiles"
    utterance_file = os.path.join(base_path, "topical_chat_test_freq.json")
    reading_set_file = os.path.join(base_path, "reading_set/test_freq.json")
    
    my_dataset = GroundTruthTopical(utterance_file, reading_set_filename = reading_set_file)
